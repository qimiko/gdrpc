...
DWORD makeRequest(std::string data, LPCWSTR url, std::string& response) {
	...
	HINTERNET hConnect = InternetConnect(hSession, _T("boomlings.com"), // i could explain how web works but no. boomlings.com is the host
	...
	while (::InternetReadFile(hRequest, szBuffer, sizeof(szBuffer) - 1, &dwRead) && dwRead) { // chunk output i guess
		szBuffer[dwRead] = 0;
...
bool getLevel(int levelid, GDlevel& level) {
  // string splitting
	...
	if (levelid == level.levelID) { // don't redo request
		return true;
	}
...
void getOfficialInfo(int id, GDlevel& level) {
	...
	switch (id) { //messy thing, i apologize
	case 1:
