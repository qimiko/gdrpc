  ...
  if (update_presence) {
    ...
    case playerState::level: {
      ...
      if (!parseGJGameLevel(gamelevel, level)) {
        ...
        details =
            formatWithLevel(playtesting.detail, level, current_best);
        ...
        small_text =
            formatWithLevel(saved.smalltext, level, current_best);
...
bool Game_Loop::get_reset_timestamp() { return this->config.editor.reset_timestamp; }

std::string Game_Loop::get_executable_name() { return this->config.settings.executable_name; }
